...
inline static bool doOverlap(Pointt l1, Pointt r1, Pointt l2, Pointt r2) {
...
class scena {
    ...
    scena(drawNS::Draw3DAPI *api, shape* xd): api(api), global(xd) {}
    ...
    void wypiszElementy() {
        for(size_t i = 0; i < ids.size(); i++) {
            if(ids[i].second == "Linia") 
                continue;
            cout << i << " " << ids[i].second << " o srodku w " << (rects[i].first.x + rects[i].second.x)/2 
                 << " " << (rects[i].first.y + rects[i].second.y)/2 << "\n";
        }
    }
    ...
    // sprawdza czy dron o id ind moze ladowac
    bool czyMozeLadowac(size_t ind) {
        if(ind >= drony.size())
            return true;
