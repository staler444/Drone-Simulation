...
class GoraSpiczasta : public shape
{
    ...
    GoraSpiczasta(shape* master): shape(master)
    {
        ...
        if(lbx > ubx) swap(ubx,lbx);
        ubx++;
        int lby = rand(15,40), uby = rand(15,40);
        if(lby > uby) swap(uby,lby);
        uby++;
        points.pb({rand(-ubx,-lbx),rand(lby,uby),0});
        points.pb({rand(-ubx,-lbx),rand(lby,uby),0});
...
class GoraDachowa : public shape
{
    ...
    {
        srand(time(NULL));
        points.pb({rand(4,XRANGE),rand(-YRANGE,-XRANGE),0});
        points.pb({rand(4,XRANGE),rand(-YRANGE,-XRANGE),0});
...
class GoraPochyla : public shape
{
    ...
    GoraPochyla(shape* master): shape(master)
    {
        ...
        points.pb({points[1][0] + wektorNormalny.first,points[1][1] + wektorNormalny.second,HEIGHT});
        points.pb({points[0][0] + wektorNormalny.first,points[0][1] + wektorNormalny.second,HEIGHT});
...
class Plaskowyz : public shape
{
    ...
    {
        srand(time(NULL));
        points.pb({rand(-2*XRANGE,-XRANGE),rand(-YRANGE,YRANGE),0});
        points.pb({points.back()[0],points.back()[1],HEIGHT});
        points.pb({rand(-2*XRANGE,-XRANGE),rand(-YRANGE,YRANGE),0});
        points.pb({points.back()[0],points.back()[1],HEIGHT});
